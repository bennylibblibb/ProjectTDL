    ...
    public class SportNewsMenu
    {
        ...
        public string GetMenuSports()
        {
            ...
            try
            {
                ...
                {
               //     sRtn += "<select name=\"CorrectNews\" onChange=\"goToNews(" + iIndex.ToString() + ",NewsInfoForm.CorrectNews[" + iIndex.ToString() + "].value)\"><option value=\"0\">請選擇</option>";
                    sMenuQuery = "select ISEQNO, CINFOTYPE, IHDRIDSTART, IHDRIDEND from NEWS_CFG where CAPPTYPE like '%" + newsAL[0] + "%' order by ISEQNO";
                    menuReader = m_MenuDB.ExecuteQuery(sMenuQuery);
                    while (menuReader.Read())
                    {
            ...
            {
                m_MenuFile.FilePath = ConfigurationManager.AppSettings["errlog"];
