	...
	public class SoccerMenuAnalysis {
		...
		public string Show() {
			...
			try {
                ...
                SportsOleReader = m_SportsDBMgr.ExecuteQuery("select gameinfo.league, gameinfo.match_cnt, gameinfo.host, gameinfo.guest, (select count(*) from gameinfo where gameinfo.league=leaginfo.alias and gameinfo.act='U' and leaginfo.leag_id in (select cleag_id from userprofile_info where iuser_id=" + uid + ")), (select count(*) from gameinfo where gameinfo.league=leaginfo.alias and gameinfo.act='U' and gameinfo.webact='X' and leaginfo.leag_id in (select cleag_id from userprofile_info where iuser_id=" + uid + ")) from gameinfo, leaginfo where gameinfo.league=leaginfo.alias and gameinfo.act='U' and leaginfo.leag_id in (select cleag_id from userprofile_info where iuser_id=" + uid + ") order by leaginfo.leag_order, leaginfo.alias, gameinfo.host, gameinfo.guest");
