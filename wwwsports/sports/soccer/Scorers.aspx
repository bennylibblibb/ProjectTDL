...
<script language="C#" runat="server">
	...
	void Page_Load(Object sender,EventArgs e) {
		Scorers scorer = new Scorers((string)Application["SoccerDBConnectionString"]);

		try {
			ScorersInformation.InnerHtml = scorer.ShowRank();
			sAlias = scorer.Alias;
			UpdateHistoryMessage((string)Application["retrieveInfoMsg"] + "射手榜(" + DateTime.Now.ToString("HH:mm:ss") + ")");
	...
	void SendScorer(Object sender,EventArgs e) {
		int iUpdated = 0;
		string sNow;
		Scorers updRank = new Scorers((string)Application["SoccerDBConnectionString"]);
...
function InputHelp() {
	...
	helpWin2Feature="Channelmode=No,Directories=No,Fullscreen=No,Location=No,MenuBar=No,Resizable=Yes,ScrollBars=No,Status=No,Titlebar=No,ToolBar=No,left=100,top=50,width=600,height=500";
	helpWindow = window.open('../help/ScorersHelp.htm', 'Help' , helpWin2Feature);
	helpWindow.focus();
}
