...
<script language="C#" runat="server">
	...
	void Page_Load(Object sender,EventArgs e) {
		LiveGoal goalInfo = new LiveGoal((string)Application["SoccerDBConnectionString"]);
	...
	void LiveGoalAction(Object sender,CommandEventArgs e) {
		int iUpdated = 0;
		string sCmd, sNow;
		LiveGoal LiveGoalUpd = new LiveGoal((string)Application["SoccerDBConnectionString"]);
...
function openOneGoal(matchcnt, oneGoalWinName) {
	...
	oneGoalWindow = window.open('perLiveGoal.aspx?matchcount=' + matchcnt,oneGoalWinName,oneGoalWinFeature);
	oneGoalWindow.focus();
}
...
function ReSort(sortType) {
	parent.content_frame.location.replace('LiveGoalRetrieval.aspx?sort=' + sortType);
}
...
function ShowHiddenMatches() {
	...
	matchWindow = window.open('HiddenMatch.aspx?type=goal', 'HiddenMatch' , matchWinFeature);
	matchWindow.focus();
}
</script>
...
<body>
	<form id="LiveGoalForm" method="post" runat="server">
		...
		<tr>
			...
			<td style="background-color:#FFFAF0" colspan="7" align="right">
				<!--
		...
		<tr>
			<td colspan="12"><asp:Button id="SortBtn" Text="儲存序號" CommandName="SORT" tabindex="500" OnCommand="LiveGoalAction" runat="server" /></td>
			<td style="background-color:#FFC39C" colspan="2" align="right"><asp:Button id="ShowBtn" Text="顯示所有" CommandName="SHOW" OnCommand="LiveGoalAction" runat="server" /></td>
		...
		<tr>
			<th align="right" colspan="15" style="background-color:#FFC39C; color:#191970">
