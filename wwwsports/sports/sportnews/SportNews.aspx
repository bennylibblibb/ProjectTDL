...
<script language="C#" runat="server">
	...
	void Page_Load(Object sender,EventArgs e) {
		SportNews newsInfo = new SportNews((string)Application["SoccerDBConnectionString"]);

		try {
			NewsInformation.InnerHtml = newsInfo.GetMessage();
			m_MsgCnt = newsInfo.MessageCount;
			m_MatchRecordCnt = newsInfo.MatchRecordCount;
			UpdateHistoryMessage((string)Application["retrieveInfoMsg"] + "其他資訊(" + DateTime.Now.ToString("HH:mm:ss") + ")");
	...
	void SendNews(Object sender,EventArgs e) {
		int iUpdated = 0;
		string sNow;
		SportNews NewsSave = new SportNews((string)Application["SoccerDBConnectionString"]);
...
%>
	function DeviceCheck() {
		...
		if (NewsInfoForm.AppID.value >= 409 && NewsInfoForm.AppID.value <= 440) {
			if (NewsInfoForm.SportMsg0.value == '') {
				alert ('Could not be empty content');
				NewsInfoForm.mustSend[0].disabled = true;
				NewsInfoForm.mustSend[1].disabled = true;
