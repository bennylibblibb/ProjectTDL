...
Remarks:
Use HOST_PLAYERNO in othresultinfo to indicate the record is sent out or not
GUEST_PLAYERNO in othresultinfo is available

Last updated:
26 July 2004, Chapman Choi

C#.NET complier statement:
csc /t:library /out:..\bin\OtherSoccerGoalDetails.dll /r:..\bin\DBManager.dll;..\bin\Files.dll;..\bin\MessageClient.dll;..\bin\SportsMessage.dll OtherSoccerGoalDetails.cs
*/
...
[assembly:AssemblyDescription("足球資訊2 -> 比數詳情")]
[assembly:AssemblyProduct("Sports.NET")]
[assembly:AssemblyTitle("Sports.NET DLL")]
[assembly:AssemblyVersion("3.1.*")]
namespace SportsUtil {
	public class OtherSoccerGoalDetails {
		...
		public OtherSoccerGoalDetails(string Connection) {
      ...
      try {
	      SQLString.Remove(0,SQLString.Length);
				SQLString.Append("select host, guest, leaglong, matchdate, matchtime from othersoccerinfo where match_cnt=");
				...
				//--------------select menber from othresultinfo
				SQLString.Remove(0,SQLString.Length);
				SQLString.Append("select count(match_cnt) from othresultinfo where match_cnt=");
				SQLString.Append(sMatchCount);
				...
				if(iDetailsCount != 0) {
					SQLString.Remove(0,SQLString.Length);
					SQLString.Append("select sent_flag, res_period, res_flag, res_time, h_scr, g_scr, player, hostpk, guestpk, host_player, guest_player, matchstate, fgs_player_no from othresultinfo where match_cnt=");
					SQLString.Append(sMatchCount);
...
*/
				...
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetails.csGetDetails(): " + ex.ToString());
				m_SportsLog.Close();
				HTMLString.Remove(0,HTMLString.Length);
		...
		public int Update() {
			...
			try {
				SQLString.Remove(0,SQLString.Length);
				SQLString.Append("select leaglong,host ,guest from othersoccerinfo where match_cnt=");
				SQLString.Append(sMatchCount);
				m_SportsOleReader = m_SportsDBMgr.ExecuteQuery(SQLString.ToString());
				if(m_SportsOleReader.Read()) {
				...
				SQLString.Remove(0,SQLString.Length);
				SQLString.Append("delete from othresultinfo where match_cnt=");
				SQLString.Append(sMatchCount);
				...
				SQLString.Remove(0,SQLString.Length);
				SQLString.Append("select count(result_id) from othresultinfo");
				icount = m_SportsDBMgr.ExecuteScalar(SQLString.ToString());
				m_SportsDBMgr.Close();
				if(icount != 0) {
					SQLString.Remove(0,SQLString.Length);
					SQLString.Append("select Max(result_id) from othresultinfo");
					icount = m_SportsDBMgr.ExecuteScalar(SQLString.ToString());
					m_SportsDBMgr.Close();
				}
...
*/
					...
					for(int i = 0; i < iDataRecords; i++) {		//判斷要循環的次數來逐次添加紀錄										
						++icount;
						SQLString.Remove(0,SQLString.Length);
						SQLString.Append("insert into othresultinfo (result_id, match_cnt, league, act, matchstate, res_flag, res_period, res_time, h_scr, g_scr, flag, alert, player, host_player, sent_flag, guest_player, fgs_player_no, hostpk, guestpk) values (");
					...
					if(arrSendToPager.Length > 0) {
						...
						try {
							...
							try {
								...
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetails.cs.Update(): Notify via MSMQ throws MessageQueueException: " + mqEx.ToString());
								m_SportsLog.Close();
								...
								m_SportsLog.Open();
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Goal Details");
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetails.cs.Update(): Notify via .NET Remoting throws exception: " + ex.ToString());
								m_SportsLog.Close();
							}
						} catch(Exception ex) {
							m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
							m_SportsLog.SetFileName(0,LOGFILESUFFIX);
							m_SportsLog.Open();
							m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetails.cs.Update(): Notify via MSMQ throws exception: " + ex.ToString());
							m_SportsLog.Close();
						}
					}	//end if
					//end modify
				...
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetails.cs: Send " + iDataRecords.ToString() + " goal details records (" + HttpContext.Current.Session["user_name"] + ")");
				m_SportsLog.Close();
				...
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetails.cs.Update(): " + ex.ToString());
				m_SportsLog.Close();
			}
