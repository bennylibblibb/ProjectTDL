...
csc /t:library /out:..\bin\ModifyTimeOfOtherGame.dll /r:..\bin\DBManager.dll;..\bin\Files.dll;..\bin\MessageClient.dll;..\bin\SportsMessage.dll ModifyTimeOfOtherGame.cs
*/
...
[assembly:AssemblyDescription("足球2資訊 -> 現場比數(修改進行時間)")]
[assembly:AssemblyProduct("Sports.NET")]
[assembly:AssemblyTitle("Sports.NET DLL")]
[assembly:AssemblyVersion("3.1.*")]
namespace SportsUtil {
	public class ModifyTimeOfOtherGame {
		...
		public ModifyTimeOfOtherGame(string Connection) {
		...
		public string GetMatch() {
			...
			SQLString.Append("select othersoccerinfo.matchdate, othersoccerinfo.matchtime, othersoccerinfo.field, othersoccergoalinfo.status, othersoccerinfo.league, othersoccerinfo.leaglong, othersoccerinfo.host, othersoccergoalinfo.h_scr, ");
			SQLString.Append("othersoccerinfo.guest, othersoccergoalinfo.g_scr, (select STARTTIME from TIMEOFOTHERGAME_INFO where IMATCH_CNT=othersoccergoalinfo.match_cnt) STARTTIME, othersoccergoalinfo.remark from othersoccerinfo, othersoccergoalinfo ");
			SQLString.Append("where othersoccerinfo.match_cnt = othersoccergoalinfo.match_cnt and othersoccergoalinfo.match_cnt=");
			...
			try {
				...
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " ModifyTimeOfOtherGame.cs.GetMatch(): " + ex.ToString());
				m_SportsLog.Close();
		...
		public int UpdateStartTime() {
			...
			try {
				...
				if(!sTimeOfGame.Equals("")) {
					...
					if(iTimeActivated > 0) {
						m_SportsDBMgr.ExecuteNonQuery("update TIMEOFOTHERGAME_INFO set STARTTIME='" + DateTime.Now.Subtract(TimeSpan.FromMinutes(dTimeOfGame)).ToString("yyyy-MM-dd HH:mm:ss") + "', CSTATUS='" + sStatus + "' where IMATCH_CNT=" + sMatchCount);
					} else {
						m_SportsDBMgr.ExecuteNonQuery("insert into TIMEOFOTHERGAME_INFO values(" + sMatchCount + ",'" + DateTime.Now.Subtract(TimeSpan.FromMinutes(dTimeOfGame)).ToString("yyyy-MM-dd HH:mm:ss") + "','" + sStatus + "')");
					}
					...
					m_SportsLog.SetFileName(0,LOGFILESUFFIX);
					m_SportsLog.Open();
					m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " ModifyTimeOfOtherGame.cs: Modify time of Other game (" + HttpContext.Current.Session["user_name"] + ")");
					m_SportsLog.Close();
				} else {
					if(iTimeActivated > 0) {
						m_SportsDBMgr.ExecuteNonQuery("update TIMEOFOTHERGAME_INFO set STARTTIME=null, CSTATUS='" + sStatus + "' where IMATCH_CNT=" + sMatchCount);
					} else {
						m_SportsDBMgr.ExecuteNonQuery("insert into TIMEOFOTHERGAME_INFO values(" + sMatchCount + ",null,'" + sStatus + "')");
					}
					...
					m_SportsLog.SetFileName(0,LOGFILESUFFIX);
					m_SportsLog.Open();
					m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " ModifyTimeOfOtherGame.cs: Reset time of game to null (" + HttpContext.Current.Session["user_name"] + ")");
					m_SportsLog.Close();
				}
				...
				if(arrSendToPager.Length>0) {
					...
					try {
						...
						try {
							...
							m_SportsLog.SetFileName(0,LOGFILESUFFIX);
							m_SportsLog.Open();
							m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " MSMQ ERROR: Modify Time of Other Game");
							m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " ModifyTimeOfOtherGame.cs.UpdateStartTime(): Notify via MSMQ throws MessageQueueException: " + mqEx.ToString());
							m_SportsLog.Close();
							...
							if(!msgClt.SendMessage((object)sptMsg)) {
								...
								m_SportsLog.SetFileName(0,LOGFILESUFFIX);
								m_SportsLog.Open();
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Modify Time of Other Game");
								m_SportsLog.Close();
							}
						}	catch(Exception ex) {
							m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
							...
							m_SportsLog.Open();
							m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Modify Time of Other Game");									
							m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " ModifyTimeOfOtherGame.cs.UpdateStartTime(): Notify via .NET Remoting throws exception: " + ex.ToString());
							m_SportsLog.Close();
						}							
					}	catch(Exception ex) {
						m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " ModifyTimeOfOtherGame.cs.UpdateStartTime(): Notify via MSMQ throws exception:  " + ex.ToString());
						m_SportsLog.Close();
					}		
				...
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " ModifyTimeOfOtherGame.cs.UpdateLiveGoals(): " + ex.ToString());
				m_SportsLog.Close();
			}
			return iRecIndex;
		}
