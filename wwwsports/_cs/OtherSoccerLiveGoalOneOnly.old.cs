...
Last updated:
18 Oct 2004, (Fanny Cheung) rewrite for othersoccergoal
...
namespace SportsUtil {
	public class OtherSoccerLiveGoalOneOnly {
		...
		public string GetLiveGoal() {
			...
			SQLString.Append("select goal.status, goal.match_cnt, game.LEAGUE, game.leaglong, game.matchdate, game.matchtime, game.field, game.host, goal.h_scr, game.guest, goal.g_scr from othersoccerinfo game, othersoccergoalinfo goal where game.match_cnt=goal.match_cnt and goal.match_cnt=");
...
*/
					...
					iMatchCount = m_SportsOleReader.GetInt32(1);
					...
					//Host Name
					sHost = m_SportsOleReader.GetString(7).Trim();
					...
					//Guest Name
					sGuest = m_SportsOleReader.GetString(9).Trim();
		...
		public int UpdateLiveGoal(string sMode) {
			...
			if(sMatchCount != null) {
				if(!sMatchCount.Equals("")) {
					try {
						if(sMode.Equals("HCG")) {	//Increment Host Goal
						...
						/*
						else if(sMode.Equals("STA")) {	//Update status period
							sOriginalStatus = m_SportsDBMgr.ExecuteQueryString("select STATUS from goal where MATCH_CNT=" + sMatchCount);
							m_SportsDBMgr.Close();
						...
						*/
						...
						//update othersoccer live goal table (goal)
						SQLString.Remove(0,SQLString.Length);
						...
						*/
						...
						//Insert log into LOG_OTHERSOCCERLIVEGOAL
						SQLString.Remove(0,SQLString.Length);
...
*/
				...
				}//end-if: MatchCount is empty or not
			} else {
				iRecUpd = 0;
			}//end-if: check MatchCount is null or not
/*
			if(iRecUpd > 0) {
