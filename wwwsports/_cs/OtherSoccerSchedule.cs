/*
Objective:
Import soccer schedule to 亞洲2 Sports
Enquiry by league, date is allowed

Last updated:
5 Apr 2005 by Paddy, try catch the case of get Null String of hostname/guestname
15 Nov 2004 by Fanny, Increment orderId into 3 digits
7 Oct 2004 by Fanny

C#.NET complier statement:
csc /t:library /out:..\bin\OtherSoccerSchedule.dll /r:..\bin\DBManager.dll;..\bin\Win32INI.dll;..\bin\Files.dll OtherSoccerSchedule.cs
*/
...
[assembly:AssemblyDescription("足球資訊 -> 匯入賽程到亞洲2體育")]
[assembly:AssemblyProduct("Sports.NET")]
[assembly:AssemblyTitle("Sports.NET DLL")]
[assembly:AssemblyVersion("1.0.*")]
namespace SportsUtil {
	public class OtherSoccerSchedule {
		...
		public OtherSoccerSchedule(string Connection) {
...
*/
				...
				m_SportsOleReader.Close();
				m_SportsDBMgr.Close();
				sEnquiryOut += sYearOption + sMonthOption + sDateOption + "<input type=\"button\" value=\"顯示\" onClick=\"filterSchedule('OtherSoccerSchedule.aspx?leagID=' + ScheduleForm.leagID.value + '&Year=' + ScheduleForm.Year.value + '&Month=' + ScheduleForm.Month.value + '&Date=' + ScheduleForm.Date.value)\"></th</tr>";

				HTMLString.Append(sEnquiryOut);
				HTMLString.Append("<tr align=\"center\" style=\"background-color:#FF9428; color=#FFF5EE\"><th>賽事日期</th><th>聯賽</th><th>賽事</th><th><input type=\"checkbox\" name=\"all\" value=\"1\" onClick=\"selectAll()\">全選</th></tr>");
				SQLString.Remove(0,SQLString.Length);
				...
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerSchedule.cs.list(): " + ex.ToString());
				m_SportsLog.Close();
				HTMLString.Remove(0,HTMLString.Length);
		...
		public int import() {
			...
			try {
				...
				SQLString.Remove(0,SQLString.Length);
				SQLString.Append("delete from OTHERSOCCERINFO where ACT='D'");
				m_SportsDBMgr.ExecuteNonQuery(SQLString.ToString());
				m_SportsDBMgr.Close();
				SQLString.Remove(0,SQLString.Length);
				SQLString.Append("delete from OTHERSOCCERGOALINFO where ACT='D'");
				m_SportsDBMgr.ExecuteNonQuery(SQLString.ToString());
				m_SportsDBMgr.Close();

				//lookup remote cofig ini files
				...
				if(m_LoggedOn) {
					if(DuplicateToken(m_Token, SecurityImpersonation, ref m_TokenDuplicate) != 0) {
						...
						if(m_ImpersonationContext != null) {
							...
							m_SportsLog.SetFileName(0,LOGFILESUFFIX);
							m_SportsLog.Open();
							m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerSchedule.cs.import(): Impersonate success.");
							m_SportsLog.Close();
						} else {
							m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
							m_SportsLog.SetFileName(0,LOGFILESUFFIX);
							m_SportsLog.Open();
							m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerSchedule.cs.import(): m_ImpersonationContext is null.");
							m_SportsLog.Close();
						}
					} else {
						m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerSchedule.cs.import(): DuplicateToken error, code = " + Marshal.GetLastWin32Error());
						m_SportsLog.Close();
					}
				} else {
					m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
					m_SportsLog.SetFileName(0,LOGFILESUFFIX);
					m_SportsLog.Open();
					m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerSchedule.cs.import(): LogonUser error, code = " + Marshal.GetLastWin32Error());
					m_SportsLog.Close();
				}
				...
				for(int itemIdx = 0; itemIdx < iImportCount; itemIdx++) {
					...
					try {
						...
						*/
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerSchedule.cs.import(): Duplicate record existed.");
				...
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerSchedule.cs.import(): Import " + iImported.ToString() + " matches (" + HttpContext.Current.Session["user_name"] + ")");
				m_SportsLog.Close();
			} catch(Exception ex) {
				iImported = -1;
				m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerSchedule.cs.import(): " + ex.ToString());
				m_SportsLog.Close();
			}
