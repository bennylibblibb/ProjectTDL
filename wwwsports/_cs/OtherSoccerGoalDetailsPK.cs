...
csc /t:library /out:..\bin\OtherSoccerGoalDetailsPK.dll /r:..\bin\DBManager.dll;..\bin\Files.dll;..\bin\MessageClient.dll;..\bin\SportsMessage.dll OtherSoccerGoalDetailsPK.cs
*/
...
[assembly:AssemblyDescription("足球資訊2 -> 比數詳情(十二碼)")]
[assembly:AssemblyProduct("Sports.NET")]
[assembly:AssemblyTitle("Sports.NET DLL")]
[assembly:AssemblyVersion("3.1.*")]
namespace SportsUtil {
	public class OtherSoccerGoalDetailsPK {
		...
		public OtherSoccerGoalDetailsPK(string Connection) {
		...
		public string ShowPK() {
			...
			try {
				...
				SQLString.Append("select host, guest from othersoccerinfo, othresultinfo where othersoccerinfo.match_cnt=");
				...
				SQLString.Remove(0,SQLString.Length);
				SQLString.Append("select Hostpk, Guestpk, FLAG from othresultinfo where match_cnt=");
				SQLString.Append(sMatchCnt);
				SQLString.Append("and act='U'");
				m_SportsOleReader = m_SportsDBMgr.ExecuteQuery(SQLString.ToString());
				if(m_SportsOleReader.Read()) {
		    ...
		    /********************************
			  ...
			  HTMLString.Append(">主隊先射</th></tr>");
				...
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetailsPK.cs.ShowPK(): " + ex.ToString());
				m_SportsLog.Close();
		...
		public int UpdatePK() {
  		  ...
  		  ********************************/
  		  SQLString.Remove(0,SQLString.Length);
				SQLString.Append("update othresultinfo set HOSTPK='");
				...
				if(arrSendToPager.Length > 0) {
					...
					if(iDetailsCount > 0) {
						...
						SQLString.Remove(0,SQLString.Length);
						SQLString.Append("select alert, league, ACT, matchstate, res_flag, res_period, res_time, h_scr, g_scr, host_player, guest_player, player, fgs_player_no from othresultinfo where match_cnt=");
						SQLString.Append(sMatchCount);
	  		  	...
	  		  	sBatchJob = DateTime.Now.ToString("yyMMddHHmmss_fffffff") + "." + HttpContext.Current.Session["user_name"].ToString() + "." + arrMsgType[15] + ".ini";
						for(int i=0;i<iresultnumber;i++) {
							...
							LogSQLString.Append("insert into LOG_GOALDETAILS (TIMEFLAG, IITEMSEQ_NO, SECTION, LEAGUE, HOST, GUEST, ACT, ALERT, CURRENT_STATUS, PK_FIRST, SCORE_REDFLAG, SCORE_STATUS, SCORE_TIME, SCORE_HOST_GOAL, SCORE_GUEST_GOAL, SCORE_PLAYER, SCORE_HOST_PK, SCORE_GUEST_PK, BATCHJOB) values ('");
		  	  	...
		  	  	//Tell MessageDispatcher to generate Goal Details PK INI and notify other processes
						...
						try {
							...
							try {
								...
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetailsPK.cs.UpdatePK(): Notify via MSMQ throws MessageQueueException: " + mqEx.ToString());
								m_SportsLog.Close();
								...
								m_SportsLog.SetFileName(0,LOGFILESUFFIX);
								m_SportsLog.Open();
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Other Soccer Goal Details PK");
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetailsPK.cs.UpdatePK(): Notify via .NET Remoting throws exception: " + ex.ToString());
								m_SportsLog.Close();
							}
						} catch(Exception ex) {
							m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
							m_SportsLog.SetFileName(0,LOGFILESUFFIX);
							m_SportsLog.Open();
							m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetailsPK.cs.UpdatePK(): Notify via MSMQ throws exception: " + ex.ToString());
							m_SportsLog.Close();
						}
					}
				} // end modify
				...
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetailsPK.cs.UpdatePK(): " + ex.ToString());
				m_SportsLog.Close();
				iDetailsCount = -1;
			}
			...
			m_SportsLog.SetFileName(0,LOGFILESUFFIX);
			m_SportsLog.Open();
			m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerGoalDetailsPK.cs: Send " + iDetailsCount.ToString() + " PK details records (" + HttpContext.Current.Session["user_name"] + ")");
			m_SportsLog.Close();
