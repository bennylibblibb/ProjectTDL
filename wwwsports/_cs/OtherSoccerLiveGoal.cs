...
Last updated:
01 Aug 2005, Chris Tsui - Add JC Combo device
...
01 Sep 2004, (Fanny) no need to append goal remark to mango goal alert message
09 Aug 2004, Chapman Choi
09 Aug 2004, Convert league and tem to HKJC format for Goal Alert
...
(With Replicator) csc /t:library /out:..\bin\OtherSoccerLiveGoal.dll /r:..\bin\DBManager.dll;..\bin\Files.dll;..\bin\MessageClient.dll;..\bin\Replicator.dll;..\bin\SportsMessage.dll OtherSoccerLiveGoal.cs

Without Replicator (Current production):
csc /t:library /out:..\bin\OtherSoccerLiveGoal.dll /r:..\bin\DBManager.dll;..\bin\Files.dll;..\bin\MessageClient.dll;..\bin\SportsMessage.dll OtherSoccerLiveGoal.cs
*/
...
[assembly:AssemblyDescription("足球2資訊 -> 現場比數")]
[assembly:AssemblyProduct("Sports.NET")]
[assembly:AssemblyTitle("Sports.NET DLL")]
[assembly:AssemblyVersion("3.1.*")]
namespace SportsUtil {
	public class OtherSoccerLiveGoal {
		...
		public OtherSoccerLiveGoal(string Connection) {
		...
		public string GetLiveGoals() {
			...
			SQLString.Append("select distinct othersoccerinfo.league from othersoccerinfo, othersoccergoalinfo, leaginfo where othersoccerinfo.match_cnt = othersoccergoalinfo.match_cnt and othersoccerinfo.league = leaginfo.alias and othersoccergoalinfo.webact = 'V' and othersoccergoalinfo.act='U' and leaginfo.leag_id in (select cleag_id from userprofile_info where iuser_id=");
			...
			SQLString.Remove(0,SQLString.Length);
			SQLString.Append("select othersoccergoalinfo.match_cnt, othersoccergoalinfo.order_id, othersoccerinfo.matchdate, othersoccerinfo.matchtime, othersoccerinfo.field, othersoccergoalinfo.status, othersoccerinfo.league, othersoccerinfo.leaglong, othersoccerinfo.host, othersoccergoalinfo.h_scr, othersoccerinfo.guest, othersoccergoalinfo.g_scr, (select STARTTIME from TIMEOFOTHERGAME_INFO where IMATCH_CNT=othersoccergoalinfo.match_cnt) STARTTIME, othersoccergoalinfo.remark from othersoccerinfo, othersoccergoalinfo, leaginfo ");
			SQLString.Append("where othersoccerinfo.match_cnt = othersoccergoalinfo.match_cnt and othersoccerinfo.league = leaginfo.alias and othersoccergoalinfo.webact = 'V' and othersoccergoalinfo.act='U' ");
			...
			if(sSortType.Equals("0")) {		//order by league and then match date and time, order_id
				SQLString.Append("order by leaginfo.ALIAS, othersoccerinfo.matchdate, othersoccerinfo.matchtime, othersoccergoalinfo.order_id");
			} else {		//order by order id
				SQLString.Append("order by othersoccergoalinfo.order_id, othersoccerinfo.matchdate, othersoccerinfo.matchtime, leaginfo.ALIAS");
			}
			...
			try {
				...
				while(m_SportsOleReader.Read()) {
					...
					//Match Status
					iArrayIndex = 0;
					sMatchStatus = m_SportsOleReader.GetString(5).Trim();
					...
					//Time Of Game
					HTMLString.Append("<td><font size=\"1\"><a href=\"ModifyTimeOfOtherGame.aspx?matchcnt=");
					HTMLString.Append(iMatchCount.ToString());
					HTMLString.Append("\">");
				...
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.GetLiveGoals(): " + ex.ToString());
				m_SportsLog.Close();
			...
			try {
				switch(sType) {
					case "SEND":	//case: Sending match
						...
						sBatchJob = DateTime.Now.ToString("yyMMddHHmmss_fffffff") + "." + HttpContext.Current.Session["user_name"].ToString() + "." + arrMsgType[40] + ".ini";
						for(iRecIndex=0;iRecIndex<iMustSendLen;iRecIndex++) {
							...
							//compare status
							sOriginalStatus = m_SportsDBMgr.ExecuteQueryString("select status from othersoccergoalinfo where match_cnt=" + arrMatchCnt[iUpdIndex]);
							m_SportsDBMgr.Close();
							iTimeActivated = m_SportsDBMgr.ExecuteScalar("select count(IMATCH_CNT) from TIMEOFOTHERGAME_INFO where IMATCH_CNT=" + arrMatchCnt[iUpdIndex]);
							m_SportsDBMgr.Close();
							if(arrStatus[iUpdIndex].Equals("1")) {	//current status = 上半
								if(sOriginalStatus.Equals("0")) {	//未開 -> 上半
									if(iTimeActivated > 0) {
										m_SportsDBMgr.ExecuteNonQuery("update TIMEOFOTHERGAME_INFO set STARTTIME=CURRENT_TIMESTAMP, CSTATUS='" + arrStatus[iUpdIndex] + "' where IMATCH_CNT=" + arrMatchCnt[iUpdIndex]);
									} else {
										m_SportsDBMgr.ExecuteNonQuery("insert into TIMEOFOTHERGAME_INFO values(" + arrMatchCnt[iUpdIndex] + ",CURRENT_TIMESTAMP,'" + arrStatus[iUpdIndex] + "')");
									}
									m_SportsDBMgr.Close();
								}
							...
							//update othersoccergoalinfo's SQL start
							SQLString.Remove(0,SQLString.Length);
							SQLString.Append("update othersoccergoalinfo set");

							//update othersoccergoalinfo's host score
							...
							//update othersoccergoalinfo's guest score
							...
							//update othersoccergoalinfo's match status
							SQLString.Append(", STATUS='" + arrStatus[iUpdIndex] + "'");

							//update othersoccergoalinfo's remarks
							...
							//update for specific othersoccergoalinfo's match count
...
*/
								...
								try {
									...
									try {
										...
										m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.UpdateLiveGoals(): Notify (LiveGoal - Mango) via MSMQ throws MessageQueueException: " + mqEx.ToString());
										m_SportsLog.Close();
										...
										m_SportsLog.Open();
										m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Live Goal - Mango");
										m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.UpdateLiveGoals(): Notify (LiveGoal - Mango) via .NET Remotingthrows exception: " + ex.ToString());
										m_SportsLog.Close();
									}
								}	catch(Exception ex) {
									m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
									m_SportsLog.SetFileName(0,LOGFILESUFFIX);
									m_SportsLog.Open();
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.UpdateLiveGoals(): Notify (LiveGoal - Mango) via MSMQ throws exception: " + ex.ToString());
									m_SportsLog.Close();
								}
							...
							if(arrSendToPager.Length>0 && iMustSendLen>0) {
								...
								//Time Of Game
								m_SportsOleReader = m_SportsDBMgr.ExecuteQuery("select STARTTIME from TIMEOFOTHERGAME_INFO where IMATCH_CNT=" + arrMatchCnt[iUpdIndex]);
								...
								//Insert log into LOG_OTHERSOCCER_LIVEGOAL
								SQLString.Remove(0,SQLString.Length);
								SQLString.Append("insert into LOG_OTHERSOCCER_LIVEGOAL (TIMEFLAG, SECTION, LEAGUE, HOST, GUEST, ACT, MATCHDATE, MATCHTIME, MATCHFIELD, H_GOAL, G_GOAL, H_CONFIRM, G_CONFIRM, SONGID, ALERT, STATUS, COMMENT, TIMEOFGAME, BATCHJOB) values ('");
						...
						if(arrSendToPager.Length>0 && iMustSendLen > 0) {
							...
							try {
								...
								try {
									...
									m_SportsLog.Open();
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " MSMQ ERROR: Live Goal");
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.UpdateLiveGoals(): Notify (LiveGoal) via MSMQ throws MessageQueueException: " + mqEx.ToString());
									m_SportsLog.Close();
									...
									m_SportsLog.Open();
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Live Goal");
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.UpdateLiveGoals(): Notify (LiveGoal) via .NET Remotingthrows exception: " + ex.ToString());
									m_SportsLog.Close();
								}
							}	catch(Exception ex) {
								m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
								m_SportsLog.SetFileName(0,LOGFILESUFFIX);
								m_SportsLog.Open();
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.UpdateLiveGoals(): Notify (LiveGoal) via MSMQ throws exception: " + ex.ToString());
								m_SportsLog.Close();
							}
						}
						//modified end
						...
						if(iLiveOdds>0 && arrSendToPager.Length>0) {
							...
							try {
								...
								try {
									...
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.UpdateLiveGoals(): Notify (LiveOdds) via MSMQ throws MessageQueueException: " + mqEx.ToString());
									m_SportsLog.Close();
									...
									m_SportsLog.Open();
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Live Odds");
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.UpdateLiveGoals(): Notify (LiveOdds) via .NET Remoting throws exception: " + ex.ToString());
									m_SportsLog.Close();
								}
							}	catch(Exception ex) {
								m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
								m_SportsLog.SetFileName(0,LOGFILESUFFIX);
								m_SportsLog.Open();
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.UpdateLiveGoals(): Notify (LiveOdds) via MSMQ throws exception: " + ex.ToString());
								m_SportsLog.Close();
							}
...
*/
					...
					case "SORT":	//case: Sorting match order
						for(iRecIndex=0;iRecIndex<arrOrderID.Length;iRecIndex++) {
							...
							SQLString.Remove(0,SQLString.Length);
							SQLString.Append("update othersoccergoalinfo set ORDER_ID='");
							...
							m_SportsDBMgr.Close();
							SQLString.Remove(0,SQLString.Length);
							SQLString.Append("select count(*) from OTHERSOCCERINFO where MATCH_CNT=");
							SQLString.Append(arrMatchCnt[iRecIndex].Trim());
							if (m_SportsDBMgr.ExecuteScalar(SQLString.ToString()) > 0) {
								SQLString.Remove(0,SQLString.Length);
								SQLString.Append("update OTHERSOCCERINFO set MATCH_ID='");
								SQLString.Append(sOrder);
								SQLString.Append("' where MATCH_CNT=");
								SQLString.Append(arrMatchCnt[iRecIndex].Trim());
								m_SportsDBMgr.ExecuteNonQuery(SQLString.ToString());
...
*/
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs: Sort " + iRecIndex.ToString() + " goals (" + HttpContext.Current.Session["user_name"] + ")");
						m_SportsLog.Close();
						break;
					case "HIDE":	//case: Hidden specific match
						for(iRecIndex=0;iRecIndex<iHiddenLen;iRecIndex++) {
							...
							if(iRecIndex == 0) {
								SQLString.Remove(0,SQLString.Length);
								SQLString.Append("update othersoccergoalinfo set WEBACT='X' where MATCH_CNT=");
								SQLString.Append(sTempItem);
							}	else {
...
*/
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs: Hide " + iRecIndex.ToString() + " goals (" + HttpContext.Current.Session["user_name"] + ")");
						m_SportsLog.Close();
						break;
					case "SHOW":	//case: Set all matches to visible
						SQLString.Remove(0,SQLString.Length);
						SQLString.Append("update othersoccergoalinfo set WEBACT='V'");
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs: " + HttpContext.Current.Session["user_name"] + " set " + iRecIndex.ToString() + " goals to visible");
						m_SportsLog.Close();
						break;
					default:
						iRecIndex = 0;
				...
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerLiveGoal.cs.UpdateLiveGoals(): " + ex.ToString());
				m_SportsLog.Close();
			}
			return iRecIndex;
		}
