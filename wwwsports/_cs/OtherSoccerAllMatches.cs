/*
Objective:
Retrieval and modify all other soccer matches information such as odds, host handicap and match field
...
(with Replicator)
csc /t:library /out:..\bin\OtherSoccerAllMatches.dll /r:..\bin\DBManager.dll;..\bin\Files.dll;..\bin\MessageClient.dll;..\bin\Replicator.dll;..\bin\SportsMessage.dll OtherSoccerAllMatches.cs

(without Replicator - current production version)
csc /t:library /out:..\bin\OtherSoccerAllMatches.dll /r:..\bin\DBManager.dll;..\bin\Files.dll;..\bin\MessageClient.dll;..\bin\SportsMessage.dll OtherSoccerAllMatches.cs
*/
...
[assembly:AssemblyDescription("足球資訊 -> 亞洲2所有賽事")]
[assembly:AssemblyProduct("Sports.NET")]
[assembly:AssemblyTitle("Sports.NET DLL")]
[assembly:AssemblyVersion("3.1.*")]
namespace SportsUtil {
	public class OtherSoccerAllMatches {
		...
		public OtherSoccerAllMatches(string Connection) {
		...
		public string GetAllMatches() {
			...
			SQLString.Append("select distinct OTHERSOCCERINFO.LEAGUE from OTHERSOCCERINFO, leaginfo where OTHERSOCCERINFO.WEBACT='V' and OTHERSOCCERINFO.ACT='U' and OTHERSOCCERINFO.league = leaginfo.alias and leaginfo.leag_id in (select cleag_id from userprofile_info where iuser_id=");
			...
			SQLString.Remove(0,SQLString.Length);
			SQLString.Append("select OTHERSOCCERINFO.MATCHDATE, OTHERSOCCERINFO.MATCHTIME, OTHERSOCCERINFO.LEAGLONG, OTHERSOCCERINFO.MATCH_CNT, OTHERSOCCERINFO.INTERVAL, OTHERSOCCERINFO.MATCH_ID, OTHERSOCCERINFO.LEAGUE, OTHERSOCCERINFO.HOST, OTHERSOCCERINFO.GUEST, OTHERSOCCERINFO.FIELD, OTHERSOCCERINFO.HOST_HANDI, OTHERSOCCERINFO.T_HANDI1, OTHERSOCCERINFO.T_HANDI2, OTHERSOCCERINFO.TY1_ODDS, OTHERSOCCERINFO.TY1_STATUS, OTHERSOCCERINFO.M_HANDI1, OTHERSOCCERINFO.M_HANDI2, OTHERSOCCERINFO.TY2_ODDS, OTHERSOCCERINFO.TY2_STATUS, OTHERSOCCERINFO.EU_W_ODDS, OTHERSOCCERINFO.EU_L_ODDS, OTHERSOCCERINFO.EU_D_ODDS from OTHERSOCCERINFO, leaginfo where OTHERSOCCERINFO.WEBACT='V' and OTHERSOCCERINFO.ACT='U' and OTHERSOCCERINFO.league = leaginfo.alias and leaginfo.leag_id in (select cleag_id from userprofile_info where iuser_id=");
			SQLString.Append(uid);
			SQLString.Append(") ");
			if(sSortType.Equals("0")) {		//order by league and then match date and time
				SQLString.Append("order by leaginfo.ALIAS, OTHERSOCCERINFO.MATCHDATE, OTHERSOCCERINFO.MATCHTIME, OTHERSOCCERINFO.MATCH_ID");
			} else {		//order by order id
				SQLString.Append("order by OTHERSOCCERINFO.MATCH_ID, OTHERSOCCERINFO.MATCHDATE, OTHERSOCCERINFO.MATCHTIME, leaginfo.ALIAS");
			}
			try {
				m_SportsOleReader = m_SportsDBMgr.ExecuteQuery(SQLString.ToString());
				while(m_SportsOleReader.Read()) {
...
*/
				...
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerAllMatches.cs.GetAllMatches(): " + ex.ToString());
				m_SportsLog.Close();
			...
			try {
				switch(sType) {
					case "SEND":	//case: Sending match
						...
						sBatchJob = DateTime.Now.ToString("yyMMddHHmmss_fffffff") + "." + HttpContext.Current.Session["user_name"].ToString() + "." + arrMsgType[37] + ".ini";
						for(iRecUpd=0;iRecUpd<iMustSendLen;iRecUpd++) {
							 ...
							 *****************************/
							SQLString.Remove(0,SQLString.Length);
							SQLString.Append("select TY1_ODDS from OTHERSOCCERINFO where MATCH_CNT=" + arrMatchCnt[iUpdIndex]);
							m_SportsOleReader = m_SportsDBMgr.ExecuteQuery(SQLString.ToString());
							...
							//update OTHERSOCCERINFO
							SQLString.Remove(0,SQLString.Length);
							SQLString.Append("update OTHERSOCCERINFO set FIELD='" + sMatchField + "', HOST_HANDI='" + sHostHandicap + "', T_HANDI1='" + sUpHandicap1 + "', T_HANDI2='" + sUpHandicap2 + "', M_HANDI1='" + sDownHandicap1 + "', M_HANDI2='" + sDownHandicap2 + "', ");
							...
							if(arrSendToPager.Length>0 && iMustSendLen>0) {
								...
								LogSQLString.Append("insert into LOG_OTHERSOCCER (TIMEFLAG, SECTION, LEAGUEALIAS, LEAGUE, MATCHDATE, MATCHTIME, MATCHFIELD, HOST, GUEST, ACT, HANDICAP, T_HANDIOWNER, M_HANDIOWNER, T_HANDI, M_HANDI, T_LIVEODDS, M_LIVEODDS, T_ODDS, M_ODDS, E_ODDS1, E_ODDS2, ALERT, INTERVAL, ODDS_TREND, BATCHJOB) values ('");
								LogSQLString.Append(sCurrentTimestamp);
								LogSQLString.Append("','OTHODDS_SPT_','");
								...
								dOriginalOdds = -1;
								dUpdatedOdds = -1;
							}	//insert into LOG_OTHERSOCCER
						}

						if(iLogUpd>0) {
							//Tell MessageDispatcher to generate Asia2 Odds INI and notify other processes
							...
							try {
								...
								try {
									...
									m_SportsLog.SetFileName(0,LOGFILESUFFIX);
									m_SportsLog.Open();
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " MSMQ ERROR: Asia2 Odds");
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerAllMatches.cs.UpdateMatches(): Notify via MSMQ throws MessageQueueException:  " + mqEx.ToString());
									m_SportsLog.Close();
									...
									if(!msgClt.SendMessage((object)sptMsg)) {
										...
										m_SportsLog.SetFileName(0,LOGFILESUFFIX);
										m_SportsLog.Open();
										m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Asia2 Odds");
										m_SportsLog.Close();
									}
								}	catch(Exception ex) {
									m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
									...
									m_SportsLog.Open();
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Asia2 Odds");
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerAllMatches.cs.UpdateMatches(): Notify via .NET Remoting throws exception: " + ex.ToString());
									m_SportsLog.Close();
								}
							}	catch(Exception ex) {
								m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
								m_SportsLog.SetFileName(0,LOGFILESUFFIX);
								m_SportsLog.Open();
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerAllMatches.cs.UpdateMatches(): Notify via MSMQ throws exception: " + ex.ToString());
								m_SportsLog.Close();
							}
						}
/*
...
/*
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerAllMatches.cs: Send " + iRecUpd.ToString() + " matches (" + HttpContext.Current.Session["user_name"] + ")");
						m_SportsLog.Close();
*/
						break;
					case "SORT":	//case: Sorting match order
						for(iRecUpd=0;iRecUpd<arrOrderID.Length;iRecUpd++) {
							...
							SQLString.Remove(0,SQLString.Length);
							SQLString.Append("update OTHERSOCCERINFO set MATCH_ID='" + sOrder + "' where MATCH_CNT=" + arrMatchCnt[iRecUpd].Trim());
							m_SportsDBMgr.ExecuteNonQuery(SQLString.ToString());
							m_SportsDBMgr.Close();
							SQLString.Remove(0,SQLString.Length);
							SQLString.Append("select count(*) from othersoccergoalinfo where MATCH_CNT=");
							SQLString.Append(arrMatchCnt[iRecUpd].Trim());
							if (m_SportsDBMgr.ExecuteScalar(SQLString.ToString()) > 0) {
								SQLString.Remove(0,SQLString.Length);
								SQLString.Append("update othersoccergoalinfo set ORDER_ID='" + sOrder + "' where MATCH_CNT=" + arrMatchCnt[iRecUpd].Trim());
								m_SportsDBMgr.ExecuteNonQuery(SQLString.ToString());
							}
							m_SportsDBMgr.Close();
							//MatchReplicator.Replicate(SQLString.ToString());
...
*/
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerAllMatches.cs: Sort " + iRecUpd.ToString() + " matches (" + HttpContext.Current.Session["user_name"] + ")");
						m_SportsLog.Close();
						break;
					case "HIDE":	//case: Hidden specific match
						for(iRecUpd=0;iRecUpd<iHiddenLen;iRecUpd++) {
							...
							if(iRecUpd == 0) {
								SQLString.Remove(0,SQLString.Length);
								SQLString.Append("update OTHERSOCCERINFO set WEBACT='X' where MATCH_CNT=" + sTempItem);
							}	else {
...
*/
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerAllMatches.cs: Hide " + iRecUpd.ToString() + " matches (" + HttpContext.Current.Session["user_name"] + ")");
						m_SportsLog.Close();
						break;
					case "SHOW":	//case: Set all matches to visible
						SQLString.Remove(0,SQLString.Length);
						SQLString.Append("update OTHERSOCCERINFO set WEBACT='V'");
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerAllMatches.cs: " + HttpContext.Current.Session["user_name"] + " set " + iRecUpd.ToString() + " matches to visible");
						m_SportsLog.Close();
						break;
					default:
						iRecUpd = 0;
				...
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " OtherSoccerAllMatches.cs.UpdateMatches(): " + ex.ToString());
				m_SportsLog.Close();
			}
