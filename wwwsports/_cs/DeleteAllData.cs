...
csc /t:library /out:..\bin\DeleteAllData.dll /r:..\bin\DBManager.dll;..\bin\Files.dll;..\bin\MessageClient.dll;..\bin\SportsMessage.dll DeleteAllData.cs
*/
...
[assembly:AssemblyDescription("重發/刪除資訊 -> 刪除傳呼機及網頁資訊")]
...
namespace SportsUtil {
	public class DeleteAllData {
		...
		public DeleteAllData(string Connection) {
		...
		public int Delete() {
			...
			try {
					...
					if(Convert.ToInt32(m_Role) >= Convert.ToInt32(sSvcRight)) {
						if(iDeleteItems > 0) {	//resend item(s)
							for(iDeleted = 0; iDeleted < iDeleteItems; iDeleted++) {
								...
								sBatchJob = DateTime.Now.ToString("yyMMddHHmmss_fffffff") + "." + HttpContext.Current.Session["user_name"].ToString() + "." + arrMsgType[14] + "." + iDeleted.ToString() + ".ini";
								SQLString.Remove(0,SQLString.Length);
								...
								try {
									...
									try {
										...
										m_SportsLog.SetFileName(0,LOGFILESUFFIX);
										m_SportsLog.Open();
										m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " MSMQ ERROR: Delete All Data");
										m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " DeleteAllData.cs.Delete(): Notify via MSMQ throws MessageQueueException:  " + mqEx.ToString());
										m_SportsLog.Close();
										...
										if(!msgClt.SendMessage((object)sptMsg)) {
											...
											m_SportsLog.SetFileName(0,LOGFILESUFFIX);
											m_SportsLog.Open();
											m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Delete All Data");
											m_SportsLog.Close();
										}
									}	catch(Exception ex) {
										m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
										...
										m_SportsLog.Open();
										m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " Remoting ERROR: Delete All Data");
										m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " DeleteAllData.cs.Delete(): Notify via .NET Remoting throws exception: " + ex.ToString());
										m_SportsLog.Close();
									}							
								}	catch(Exception ex) {
									m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
									m_SportsLog.SetFileName(0,LOGFILESUFFIX);
									m_SportsLog.Open();
									m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " DeleteAllData.cs.Delete(): Notify via MSMQ throws exception: " + ex.ToString());
									m_SportsLog.Close();
								}
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " DeleteAllData.cs: resend " + iDeleted.ToString() + " items (" + HttpContext.Current.Session["user_name"] + ")");
						m_SportsLog.Close();
					} else {
						iDeleted = -99;
						m_SportsLog.FilePath = HttpContext.Current.Application["EventFilePath"].ToString();
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " DeleteAllData.cs: " + HttpContext.Current.Session["user_name"].ToString() + " did not allow to resend pager data (" + HttpContext.Current.Session["user_name"] + ")");
						m_SportsLog.Close();
				...
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " DeleteAllData.cs.Delete(): " + ex.ToString());
				m_SportsLog.Close();
			}
