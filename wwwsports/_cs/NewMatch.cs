/*
Objective:
Add a new match to Asia Sports
...
csc /t:library /out:..\bin\NewMatch.dll /r:..\bin\DBManager.dll;..\bin\Win32INI.dll;..\bin\Files.dll;..\bin\Replicator.dll NewMatch.cs
csc /t:library /out:..\bin\NewMatch.dll /r:..\bin\DBManager.dll;..\bin\Win32INI.dll;..\bin\Files.dll NewMatch.cs
*/
...
namespace SportsUtil {
	public class NewMatch {
		...
		public NewMatch(string Connection) {
		...
		public string InitFields() {
			...
			try {
				...
				HTMLString.Append("\"><tr style=\"background-color:#DDA0DD\"><th colspan=6 align=\"left\">新增<font color=#FFD700>");
				HTMLString.Append(sLeag);
				...
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " NewMatch.cs.InitFields(): " + ex.ToString());
				m_SportsLog.Close();
				...
				if(m_LoggedOn) {
					if(DuplicateToken(m_Token, SecurityImpersonation, ref m_TokenDuplicate) != 0) {
						...
						if(m_ImpersonationContext != null) {
							...
							m_SportsLog.SetFileName(0,LOGFILESUFFIX);
							m_SportsLog.Open();
							m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " NewMatch.cs.Add(): Impersonate success.");
							m_SportsLog.Close();
						} else {
							m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
							m_SportsLog.SetFileName(0,LOGFILESUFFIX);
							m_SportsLog.Open();
							m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " NewMatch.cs.Add(): m_ImpersonationContext is null.");
							m_SportsLog.Close();
						}
					} else {
						m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " NewMatch.cs.Add(): DuplicateToken error, code = " + Marshal.GetLastWin32Error());
						m_SportsLog.Close();
					}
				} else {
					m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
					m_SportsLog.SetFileName(0,LOGFILESUFFIX);
					m_SportsLog.Open();
					m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " NewMatch.cs.Add(): LogonUser error, code = " + Marshal.GetLastWin32Error());
					m_SportsLog.Close();
				}
...
*/
				...
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " NewMatch.cs: Add " + iRecUpd.ToString() + " matches (" + HttpContext.Current.Session["user_name"] + ")");
				m_SportsLog.Close();
			} catch(Exception ex) {
				m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
				m_SportsLog.SetFileName(0,LOGFILESUFFIX);
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " NewMatch.cs.Add(): " + ex.ToString());
				m_SportsLog.Close();
			}
