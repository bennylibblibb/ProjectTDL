...
csc /t:library /out:..\bin\JCComboPlayers.dll /r:..\bin\DBManager.dll;..\bin\Files.dll JCComboPlayers.cs
*/
...
[assembly:AssemblyDescription("足球資訊 -> JCCombo球員")]
[assembly:AssemblyProduct("Sports.NET")]
[assembly:AssemblyTitle("Sports.NET DLL")]
[assembly:AssemblyVersion("1.0.*")]
namespace SportsUtil {
	public class JCComboPlayers {
		...
		public JCComboPlayers(string Connection) {
		...
		public string GetTeamPlayers() {
			...
			try {
				...
				iRec = 0;
				retrieveQuery = "select IPLAYER_NO, IPOS, CPLAYER_NAME, CENGNAME, CCOUNTRY, IROSTER from JCCOMBO_PLAYERS_INFO where TEAM_ID=" + sTeamID + " order by IPLAYER_NO, IPOS, CPLAYER_NAME";
				m_SportsOleReader = m_SportsDBMgr.ExecuteQuery(retrieveQuery);
				while(m_SportsOleReader.Read()) {
				...
				m_SportsLog.Open();
				m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " JCComboPlayers.cs.GetTeamPlayers(): " + ex.ToString());
				m_SportsLog.Close();
			...
			switch(sType) {
				case "MOD":	//case: modify players information
					...
					try {
						...
						playerQuery = "delete from JCCOMBO_PLAYERS_INFO where TEAM_ID=" + sTeamID;
						...
						for(iIdx=0;iIdx<arrName.Length;iIdx++) {
							...
							if(!sPlayerName.Equals("")) {
								playerQuery = "insert into JCCOMBO_PLAYERS_INFO values(" + sTeamID + ",";
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " JCComboPlayers.cs: insert " + iRecUpd.ToString() + " players with team ID=" + sTeamID + " (" + HttpContext.Current.Session["user_name"] + ")");
						m_SportsLog.Close();
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " JCComboPlayers.cs.Update().MOD: " + OleDbEx.ToString());
						m_SportsLog.Close();
					}	catch(Exception ex) {
						iRecUpd = -1;
						m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " JCComboPlayers.cs.Update().MOD: " + ex.ToString());
						m_SportsLog.Close();
					}
					break;
					...
					try {
						...
						for(int i = 0; i < arrDelete.Length; i++) {
							if(!arrDelete[i].Equals("")) {
								playerQuery = "delete from JCCOMBO_PLAYERS_INFO where TEAM_ID=" + sTeamID + " AND CPLAYER_NAME='" + arrDelete[i] + "'";
								m_SportsDBMgr.ExecuteNonQuery(playerQuery);
								m_SportsDBMgr.Close();
								iRecUpd++;
								...
								m_SportsLog.SetFileName(0,LOGFILESUFFIX);
								m_SportsLog.Open();
								m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " JCComboPlayers.cs: delete " + arrDelete[i] + " with Team ID: " + sTeamID + " (" + HttpContext.Current.Session["user_name"] + ")");
								m_SportsLog.Close();
							}
						}
					}	catch(Exception ex) {
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " JCComboPlayers.cs.Update().DEL: " + ex.ToString());
						m_SportsLog.Close();
					}
					break;
				case "COPY":	//case: copy players information w.r.t. team id
					try {
						playerQuery = "select count(*) from JCCOMBO_PLAYERS_INFO where TEAM_ID=" + sTeamID;
						iRecCnt = m_SportsDBMgr.ExecuteScalar(playerQuery);
						if (iRecCnt > 0) {
							playerQuery = "delete from JCCOMBO_PLAYERS_INFO where TEAM_ID=" + sTeamID;
							m_SportsDBMgr.ExecuteNonQuery(playerQuery);
							m_SportsDBMgr.Close();
						...
						iRecCnt = 0;
						playerQuery = "select count(*) from PLAYERS_INFO where TEAM_ID=" + sTeamID;
						iRecCnt = m_SportsDBMgr.ExecuteScalar(playerQuery);
						if (iRecCnt > 0) {
							playerQuery = "INSERT INTO JCCOMBO_PLAYERS_INFO (TEAM_ID, IPLAYER_NO, IPOS, CPLAYER_NAME, CCOUNTRY, IROSTER, CENGNAME) select TEAM_ID, IPLAYER_NO, IPOS, CPLAYER_NAME, CCOUNTRY, IROSTER, CENGNAME from PLAYERS_INFO where TEAM_ID=" + sTeamID;
							m_SportsDBMgr.ExecuteNonQuery(playerQuery);
							m_SportsDBMgr.Close();
							iRecUpd++;
						}
						...
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " JCComboPlayers.cs: copy with Team ID: " + sTeamID + " (" + HttpContext.Current.Session["user_name"] + ")");
						m_SportsLog.Close();
					} catch(Exception ex) {
						iRecUpd = -1;
						m_SportsLog.FilePath = HttpContext.Current.Application["ErrorFilePath"].ToString();
						m_SportsLog.SetFileName(0,LOGFILESUFFIX);
						m_SportsLog.Open();
						m_SportsLog.Write(DateTime.Now.ToString("HH:mm:ss") + " JCComboPlayers.cs.Update().COPY: " + ex.ToString());
						m_SportsLog.Close();
					}
					break;
			}
