...
Last updated:
15 Nov 2004, (Fanny) Increment orderId into 3 digits
18 Oct 2004, (Fanny) re-write for othersoccerlivegoal
	...
	public class OtherSoccerLiveGoal {
		...
		public string GetLiveGoals() {
			...
			if(sSortType.Equals("0")) {		//order by league and then match date and time, order_id
				SQLString.Append("order by leaginfo.leag_order, othersoccerinfo.matchdate, othersoccerinfo.matchtime, othersoccergoalinfo.order_id");
			} else {		//order by order id
				SQLString.Append("order by othersoccergoalinfo.order_id");
			}
			...
			try {
				...
				while(m_SportsOleReader.Read()) {
					...
					//Host Score Visibility (Disabled Field)
					iArrayIndex = 0;
					HTMLString.Append("<select disabled=\"true\" name=\"hostValidity\" onChange=\"onHost_VChanged(");
					HTMLString.Append(m_RecordCount.ToString());
					...
					//Guest Score Validity (Diabled Field)
					iArrayIndex = 0;
					HTMLString.Append("<select disabled=\"true\" name=\"guestValidity\" onChange=\"onGuest_VChanged(");
					HTMLString.Append(m_RecordCount.ToString());
					HTMLString.Append(")\"><option value=1>");
					HTMLString.Append(validityArray[1]);
					foreach(String sItem in validityArray) {
					...
					//Song ID (Disabled Field)
					HTMLString.Append("<td><select disabled=\"true\" type=\"hidden\" name=\"song\" onChange=\"onSongChanged(");
					HTMLString.Append(m_RecordCount.ToString());
					HTMLString.Append(")\">");
					...
					//Remark (Hidden Field)
					if(m_SportsOleReader.IsDBNull(12)) sRecordStr = "";
					else sRecordStr = m_SportsOleReader.GetString(12).Trim();
					HTMLString.Append("<td><input disabled=\"true\" type=\"text\" name=\"remark\" maxlength=\"9\" size=\"5\" value=\"");
					HTMLString.Append(sRecordStr);
			...
			try {
				switch(sType) {
					case "SEND":	//case: Sending match
						...
						string[] arrStatus, arrMatchDate, arrMatchTime, arrMatchField, arrScore_Host, arrScore_Guest;
						//string[] arrSongID, arrRemark, arrValidity_Host, arrValidity_Guest;
						string[] arrAlias, arrLeague, arrHost, arrGuest, arrAlert, arrShortStatus;
