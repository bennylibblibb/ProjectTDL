...
eventLogPath = "D:\Projects\wwwsports\event_log\web_hk_evt." & Year(Date) & Month(Date) & Day(Date) & ".log"
errorLogPath = "D:\Projects\wwwsports\error_log\web_hk_err." & Year(Date) & Month(Date) & Day(Date) & ".log"
...
If Err.Number <> 0 Then
	writeErrorLog("Create SOC connection error: " & Err.Description & " [" & Err.Number & "]" )
	Err.Clear
End If
...
writeEventLog("Prepare SOC housekeep SQL: " & sSQL)
rootRS.Open sSQL, rootConn

If Err.Number <> 0 Then
	writeErrorLog("Prepare SOC housekeep items SQL error: " & Err.Description & " [" & Err.Number & "]" )
	Err.Clear
End If
...
If Not rootRS.EOF Then
	...
	Do While Not rootRS.EOF
		...
		writeEventLog("Housekeeping SOC Match <ID: " & MatchCount & ", Date: " & rootRS("MATCHDATE") & ", Time: " & rootRS("MATCHTIME") & ", Alias: " & Trim(rootRS("LEAGUE")) & ", Host: " & Trim(rootRS("HOST")) & ", Guest: " & Trim(rootRS("GUEST")) & ">")
	...
	Loop
Else
	writeEventLog("No SOC match to housekeep, cut off: " & prune_date & " " & prune_time)
End If
