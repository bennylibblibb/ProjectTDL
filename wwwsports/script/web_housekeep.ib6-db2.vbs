...
'connection to DB2@192.168.102.5
conn.ConnectionString = "DSN=DEV102SC;UID=wgdb;PWD=ibmdb2"
conn.Open
rs.Open "delete from GAMEINFO where ACT='D'", conn
rs.Open "delete from GOALINFO where ACT='D'", conn
sSQL = "delete from GAMEINFO where (cast(MATCHDATE as integer) + cast(MATCHTIME as integer)/10000)<" & (currentdate + currenttime/10000)
rs.Open sSQL, conn
sSQL = "delete from GOALINFO where (cast(MATCHDATE as integer) + cast(MATCHTIME as integer)/10000)<" & (currentdate + currenttime/10000)
rs.Open sSQL, conn
conn.Close
