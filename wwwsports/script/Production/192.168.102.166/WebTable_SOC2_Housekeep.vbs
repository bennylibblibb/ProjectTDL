...
sSQL = "SELECT MATCH_CNT, MATCHDATE, MATCHTIME, LEAGUE, HOST, GUEST FROM OTHERSOCCERINFO where (MATCHDATE + MATCHTIME/10000)<'" & (prune_date + prune_time/10000) & "' order by MATCH_CNT"
...
If Not rootRS.EOF Then
	...
	Do While Not rootRS.EOF
		...
		sSQL = "DELETE FROM OTHERSOCCERGOALINFO WHERE MATCH_CNT=" & MatchCount
		pruneRS.Open sSQL, rootConn
		sSQL = "DELETE FROM OTHERSOCCERINFO WHERE MATCH_CNT=" & MatchCount
		pruneRS.Open sSQL, rootConn
		rootRS.MoveNext
	Loop
Else
